...
class get_mgeo :
    ...
    def getAllNode(self):
        ...
        for node_idx in self.nodes :
            buf = Point32()
            buf.x = self.nodes[node_idx].point[0]
            buf.y = self.nodes[node_idx].point[1]
            buf.z = self.nodes[node_idx].point[2]
            all_node.points.append(buf)
        return all_node
