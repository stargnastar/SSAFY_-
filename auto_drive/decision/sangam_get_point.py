...
class GPSIMUParser:
    def __init__(self):
        ...
        rospy.Subscriber("/point", String, self.getPoint_list_callback)
        ...
        while not rospy.is_shutdown():
            if self.is_pinpoint==True:
                self.convertLL2UTM()
                self.publishUTM()  # UTM data publish function 추가
    ...
    def convertLL2UTM(self):
        if self.is_pinpoint:
            self.pinpoint_utm_list = Float64MultiArray()
            for pp_x, pp_y in self.pinpoint_list: # 리스트의 각 요소는 (x, y) 형태
                pp_xy = self.proj_UTM(pp_y, pp_x) # 위도와 경도를 UTM으로 변환
    ...
    def getPoint_list_callback(self, msg):
        point_string = msg.data
        point_parsing = [float(coord) for coord in point_string.split(',')] # , 기준으로 파싱

        
        self.pinpoint_list = []

        for i in range(0, len(point_parsing), 2):
            x = point_parsing[i]
            y = point_parsing[i+1]
            coordinate_pair = (x, y)
            self.pinpoint_list.append(coordinate_pair)
        self.input_len = len(self.pinpoint_list) * 2
        #self.convertLL2UTM()
        self.is_pinpoint = True

    # UTM data publish function
    def publishUTM(self):
        self.pinpoint_utm_list_pub.publish(self.pinpoint_utm_list)
...
def callback(data):
    # 'data' contains the message sent from the Android app
    # Handle the data as needed
    print("good")
    pass

def listener():
    rospy.init_node('android_data_listener', anonymous=True)
